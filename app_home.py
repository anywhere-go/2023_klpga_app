...
def run_app_home():
    ...
    st.subheader('데이터 설명')
    st.markdown('2023년 5월 기준 **:green[Tour 프로 선수 121명]** 기록 데이터 입니다.')
    # st.markdown('Streamlit is **_really_ cool**.')
    # st.markdown("This text is :red[colored red], and this is **:blue[colored]** and bold.")
    st.markdown(":blue[Average number of hit (평균 타수)] -  참가대회 기간 동안 타수 평균 값.")
    st.markdown(":blue[Average Drive (평균 드라이브거리)] -  참가대회 기간 동안 티샷 드라이브 거리 평균 값.")
    st.markdown(":blue[Average Putting (평균 퍼팅수)] -  참가대회 기간 동안 퍼팅수 평균 값.")
